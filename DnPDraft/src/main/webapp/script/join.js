	...
	const ar = {
        ...
        birthday:[/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/, '잘못된 날짜']
    };
    ...
    async function usernameCheck() {
        ...
        try {
			// 서버의 응답은 메시지 + 상태코드로 구성된 객체다
			// $.ajax()는 200인 경우에 서버에서 결과 데이터를 자동으로 꺼내 msg에 대입한다
			const msg = await $.ajax('/member/id-check?username=' + value);
			$element.next().text(msg).attr('class', 'error');
			return true;
		} catch(response) {
			// 200이 아닌 경우 catch 절에 서버응답을 얻어와서 오류 메시지를 꺼낸다
			console.log(response);
			$element.next().text(response.responseText).attr('class', 'error');
			return false;
		}
		...
		/*
	        ...
	        $.ajax({
	            ...
	            data: 'username='+value,
	            success:function() {
	                // 상태코드 200 (오류가 발생하지 않았다)
	                $element.next().text('사용가능합니다').attr('class', 'ok');
	                return true;
	            }, error:function() {
	                // 상태코드가 200이 아니다
    ...
    function passwordCheck() {
        ...
        if (ar.password[0].test(value)==false){
			...
			$element.next().text('사용 가능한 비밀번호입니다').attr('class', 'ok')
			return true;	
		}
    }
    ...
    function password2Check() {
        ...
        if ($element.val()==='') {
            $element.next().text('확인용 비밀번호를 입력하시길 바랍니다').attr('class', 'error');
            return false;
        } else if (password!==value){
            $element.next().text('비밀번호가 일치하지 않습니다').attr('class', 'error');
            return false;
        } else {
			$element.next().text('비밀번호가 일치합니다').attr('class', 'ok')
            return true;
        }
    }
